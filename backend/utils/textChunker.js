 ...
 * @param {number} chunkSize - Target size per chnk (in words)
...
export const chunkText = (text, chunkSize = 500, overlap = 50) => {
  ...
  //Falback : if no chunks created, split by words
...
/**
 * Find relevant Chunks based on keyword mathcing
...
export const findRelevantChunks = (chunks, query, maxChunks = 3) => {
  ...
  if (queryWords.length === 0) {
    //Return clean objects without Mongoose metadata
  ...
  const scoredChunks = chunks.map((chunk, index) => {
    ...
    return {
      ...
      rawScore: score,
      mathchedWords: uniqueWordsFound,
    };
  });
  ...
  return scoredChunks
    ...
    .sort((a, b) => {
      ...
      if (b.mathchedWords !== a.mathchedWords) {
        return b.mathchedWords - a.mathchedWords;
      }
