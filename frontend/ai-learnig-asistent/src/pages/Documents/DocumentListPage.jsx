...
const DocumentListPage = () => {
  ...
  const fetchDocuments = async () => {
    try {
      ...
      console.log("fetch data documents:", data);
      setDocuments(data);
    } catch (error) {
  ...
  const handleConfirDelete = async () => {
    ...
    try {
      ...
      toast.error(error.message || "Failed to deleted document");
    } finally {
      setDeleting(false);
    }
  };
      ...
      {isUploadModalOpen && (
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-slate-900/50 backdrop-blur-sm">
          <div className="relative w-full max-w-lg bg-white/95 backdrop-blur-xl border border-slate-200/60 rounded-2xl shadow-2xl shadow-slate-900/20 p-8">
            {/* Close Button */}
            <button className="absolute top-6 right-6 w-8 h-8 flex items-center justify-center rounded-lg text-slate-400 hover:text-slate-600 hover:bg-slate-100 transition-all duration-200" onClick={() => setIsUploadModalOpen(false)}>
            ...
            <div className="mb-6">
              ...
              <p className="text-sm text-slate-500 mt-1">Add pdf to your library</p>
            </div>
            ...
            <form className="space-y-5" onSubmit={handleUpload}>
              ...
              <div className="space-y-2">
                ...
                <div className="relative border-2 border-dashed border-slate-300 rounded-xl bg-slate-50/50 placeholder-slate-400 text-sm font-medium transition-all duration-200">
                  <input id="file-upload" type="file" className="absolute inset-0 w-full h-full opacity-0 cursor-pointer z-10" onChange={handleFileChange} accept=".pdf" />
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-slate-900/50 backdrop-blur-sm">
          <div className="relative w-full max-w-md bg-white/95 backdrop-blur-xl border border-slate-200/60 rounded-2xl shadow-slate-900/20 p-8">
            ...
            <p className="text-sm text-slate-600 mb-6 ">
              ...
              <span className="font-semibold text-slate-900 ">{selectedDoc?.title}</span>? This action cannot be undone.
            </p>
